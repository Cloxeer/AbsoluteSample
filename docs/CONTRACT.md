...
# v7 addendum: karaoke, frequencies, autotune, notes fixes, standalone desktop

## Standalone desktop (build hygiene)
Desktop is the product; the web build is a demo only. `pnpm build` uses base `/` (embeds into the Tauri release exe); `pnpm build:web` uses base `/AbsoluteSample/` (GitHub Pages only). Never build web base into the exe. The release exe (`cargo build --release --bin absolutesample`) is fully standalone: it embeds `../dist` and needs no dev server. The Desktop shortcut launches it directly.

## Karaoke (fast 2-stem)
Engine `separate.py --mode karaoke`: runs ONLY BS-Roformer (`MODELS["vocals"]`), writes `vocals.wav` + `instrumental.wav` (the Roformer "instrumental"/"other" output), skips Demucs/drums/tag. Optional `--lead` also runs the karaoke lead model on the vocal stem → lead_vocals/backing_vocals. ~1 model pass (~75s full song on GPU).
Rust `separate_karaoke({ trackId, splitLeadBacking?: bool, lowPriority?: bool }) -> { stems: InstrumentStem[], elapsedSec, device }`. Output dir `<workdir>/karaoke/`, manifest `karaoke.json`, `state.json.karaokeScope=true`. Stems: instrumental (group "other", order 5), vocals (order 10), + lead/backing when asked. CLI `karaoke --track <id> [--lead]`. LibraryEntry gains `hasKaraoke`.
UI: a "Karaoke" button beside "Split" (secondary style). Running shows a Vocals (+ Lead/Backing) checklist. Result: a 2-track list (Instrumental, Vocals) reusing InstrumentTrack (audition, drag-select, save selection, download, Export both). No detections/confidence.

## Frequencies tab (replaces the MIDI value story for pitched audio; Notes tab stays for melody)
Engine `frequencies.py --input <wav> [--bpm]`: emits `{ spectrum:[{hz,db}] ~120 log bins 20Hz-20kHz avg magnitude in dB, bands:[{key,name,lowHz,highHz,db,sharePct}] (sub 20-60, bass 60-250, lowMid 250-500, mid 500-2k, highMid 2k-4k, presence 4k-6k, air 6k-20k), tuning:{ referenceHz, avgCentsOff (signed, vs nearest semitone at A=440), inTunePct (share of voiced frames within +/-15 cents), estimatedRefHz (best A4 that minimizes cents error, 430-450), perNote?:[{name,cents,count}] }, key:{tonic,mode,confidence,camelot}, durationSec }`.
Rust `analyze_frequencies({ path }) -> FrequencyResult` (cache `<name>.freq.json` next to wav). Command + CLI `frequencies <wav>`.
UI new tab "Frequencies" (3rd, after Notes): a spectrum curve (log-x, dB-y, filled under the line, accent), a 7-band energy bar row with dB + share and an InfoTip per band in plain words ("Sub: the deep rumble you feel more than hear"), and a Tuning card: big "avg cents off" number with a color (green within +/-5, amber to +/-15, red beyond), "In tune 82%", the estimated reference pitch ("This sits closest to A=442, a touch sharp of the 440 standard"), and a plain-words line explaining that 0 cents is perfectly in tune and even a few cents off is audible on sustained notes. Source picker like Notes (any stem/sample). Info "?" on cents and reference.

## Notes tab fixes
1. Saved samples don't read: `extract_notes` must accept an absolute path to a sample (in `~/.absolutesample/samples/**`) and the frontend NotesTab source picker must pass the sample's real `path`. Fix resolution in Rust (accept any readable wav path, not only workdir) and in the mock. 
2. Drums: when the chosen source is a drum stem (key in {drums,kick,snare,toms,hihat,ride,crash} or group "drums"), do NOT run Basic Pitch. Instead `notes.py --mode drums --bpm <bpm>`: onset-detect, quantize onsets to the 16th-note grid from bpm, return `{ drum:{ bpm, steps (bars*16, min 16), lanes:[{key,label,hits:[stepIdx...]}] } }`. For a specific kit piece → one lane named for it; for the whole "drums" stem → one lane "Drums" (kit split is a separate action). NotesTab renders a step-sequencer grid (lit cells on hits) instead of the piano roll, with the transport playhead sweeping columns, and a "Play" that triggers the stem sliced at each hit. NotesResult gains optional `drum`.

## Autotune / Voice enhancer (new tab "Autotune")
Engine `autotune.py`:
- `--mode analyze --input <wav>` -> `{ sampleRate, hopSec, f0:[{t,hz,midi,cents,voiced}], notes:[{startSec,endSec,midi,cents,confidence}], key:{tonic,mode,confidence} }`. Use WORLD (pyworld harvest) for f0 + spectral envelope + aperiodicity; segment voiced runs into notes (median pitch, min 80ms, split on >70 cents jumps); cents = deviation from nearest semitone (A=440).
- `--mode apply --input <wav> --edits <json>` where edits = `{ snapStrength:0..1, scale:[pitchClass ints]|null, transitionMs, notes:[{startSec,endSec,targetMidi}] }` -> writes `<out>` and prints `{ path }`. Build a target f0 per frame: within each edited note, target = targetMidi; where no note override, snap toward nearest scale note (or nearest semitone if scale null) by snapStrength; smooth transitions over transitionMs to avoid stepping; keep unvoiced frames unvoiced; resynth with pyworld.synthesize using the shifted f0 and original envelope/aperiodicity (preserves timbre, non-robotic). 24-bit out.
Rust `analyze_pitch({ path }) -> PitchResult` (cache `<name>.pitch.json`), `apply_autotune({ path, edits }) -> { path, peaks, durationSec }` (writes `<workdir or samples>/autotune/<name>_tuned.wav`), CLI `pitch <wav>` and `autotune <wav> --edits <json>`.
UI new tab "Autotune": source picker (stems/samples/imported vocal). Analyze → a Melodyne-style editor: X=time, Y=pitch (semitone rows, note names on the left, scale rows tinted), the raw f0 drawn as a thin cyan line, detected notes as draggable rounded blocks coloured by how far off they are (green=in tune). Interactions: drag a block vertically to set targetMidi (snaps to semitone; shows cents), a global "Snap strength" slider (0 Natural to 100 Full), a Key/Scale select (prefilled from detected key; "Chromatic" allowed), a "Tune to scale" one-click (sets every block to nearest scale note), "Reset". Transport plays the CURRENT (edited-preview is only after Apply). "Apply" runs apply_autotune and loads the result for A/B (a Compare toggle plays original vs tuned), then "Save as sample" / "Download". Plain-words helper: "Drag a note onto its line to tune it. Green means it is already in tune (0 cents)."
